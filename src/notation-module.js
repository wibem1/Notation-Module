// Notation Module v0.1.7 — app-independent ABC rendering/playback core.
export class NotationModule {
  ...
  render(){if(!window.ABCJS)throw new Error('abcjs ist nicht geladen.');const el=typeof this.paper==='string'?document.getElementById(this.paper):this.paper;const width=Math.max(320,Math.floor((el?.clientWidth||740)-24));const staffwidth=Math.max(280,Math.floor(width/this.scale));const out=window.ABCJS.renderAbc(this.paper,this.abc,{add_classes:true,scale:this.scale,staffwidth,wrap:{minSpacing:1.6,maxSpacing:2.7,preferredMeasuresPerLine:4,lastLineLimit:2}});this.visualObj=out[0]||null;this.onStatus(this.visualObj?'Partitur gerendert.':'Keine Partitur erzeugt.');return this.visualObj;}
  async play(){
